...
Native Android weather companion with a **chat-first** interface for India — farming, fishing, outdoor work, travel, and **disaster / emergency response**. Everything lives in **D:\WeatherGPT**. The old nested `WeatherGPT` folder is not the application root.

## Problem

People need trustworthy local weather guidance in their language — especially for farming, fishing, outdoor work, and disaster situational awareness — without inventing numbers, hiding missing official warnings, or requiring an always-online experience.

## Solution

WeatherGPT fuses validated multi-provider forecasts on a FastAPI backend, answers in plain language through chat, keeps Room as the Android source of truth offline, and always separates **official CAP / IMD warnings** from WeatherGPT risk estimates. Missing official data is reported as unavailable — never as “no warnings.”

**SIH disaster path:** an official-first multi-hazard briefing layers CAP warnings → heavy-rain / waterlogging disruption estimates → road & landslide connectivity estimates → local risk thresholds → IMD / NDMA / SACHET links. It does **not** invent shelters, live radar, flood inundation maps, or NDMA geological verdicts.
...
Chat is the default screen: suggested questions, typing, or Speak → confirm transcript → send. Answers lead with a decision or plain summary; supporting numbers come after. Forecast, alerts (disaster board), home score and settings remain available, but conversation is the front door.
...
FastAPI backend (validation, fusion, alerts, hazards, chat, translate)
        │
Open-Meteo (+ IFS) · OpenWeather* · WeatherAPI* · IMD probe*
        · CAP feed* · Open-Meteo Marine · ERA5 climate · OSM Overpass*
        · Groq* · BHASHINI* · Google Translate* · FCM*/SMS*
```

\* Requires credentials or network; disabled or probe-only without them. No weather API keys in the APK.

## Features

- Chat-first Q&A over verified weather tools (current, hourly, daily, alerts, score, climate, marine, infrastructure hazard, disaster briefing)
- Decision-first answers for rain, sowing / outdoor plans, timed events (e.g. “at 4 PM”), and disaster sitreps
- Multi-model fusion with explainable confidence (uncalibrated; not a probability)
- Official CAP path when configured (polygon / circle / areaDesc matching with honesty flags); local risk estimates always labelled separately
- Heavy-rain disruption + waterlogging estimates from verified hourly totals — labelled estimates, not flood models
- Road / landslide connectivity estimate from rainfall, soil moisture, DEM slope proxy, and OpenStreetMap roads / settlements
- `GET /v1/hazards/disaster-brief` + chat tool `get_disaster_briefing` for emergency-profile demos
- Alerts tab: official-first board, emergency banner, IMD / NDMA / SACHET deep links, shelter honesty
- 11 UI languages with full string packs; deterministic chat drafts in all 11; BHASHINI/Google Translation stay off without credentials
...
- Full-screen onboarding: language → occupation (including Emergency / disaster response) → location
...
| Data | Open-Meteo (forecast, geocoding, marine, ERA5, soil/DEM); optional OpenWeather, WeatherAPI, IMD, CAP; OSM Overpass for nearby roads/settlements |
...
### Quick SIH disaster demo prompts

With profile **Emergency / disaster response** (or ask in chat):

- “Give me a disaster management situation report for responders”
- “Outdoor event at 4 PM — heavy rainfall / waterlogging risk?”
- “Will nearby roads or villages be cut off by landslide risk?”
- “Why is it raining today?”

Useful APIs: `GET /v1/weather/alerts`, `GET /v1/hazards/infrastructure`, `GET /v1/hazards/disaster-brief`, `POST /v1/chat/message`.
...
| `CAP_ALERT_URL` / `CAP_ALERT_ALLOWED_HOSTS` | Trusted official CAP feed (e.g. IMD CAP RSS) |
...
Saved forecasts and messages live in Room; preferences in DataStore. Offline answers use the saved forecast and carry a stale warning. Cached official warnings remain available offline; new official warnings cannot arrive without connectivity. Low Data Mode shortens the bundle and lengthens refresh. Use Settings → Clear data to wipe local state.
...
- No live weather radar, hydrologic flood model, or shelter / evacuation-camp registry — disruption and infrastructure outputs are labelled WeatherGPT estimates.
...
- `docs/ALERTS.md` — CAP / risk / disaster-brief honesty
