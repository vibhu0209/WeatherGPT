...
interface Api {
    @GET("v1/locations/search") suspend fun search(@HttpQuery("q") query:String): SearchDto
...
@Entity(tableName="messages") data class Message(@PrimaryKey(autoGenerate=true) val id:Long=0, val role:String, val text:String, val language:String, val timestamp:Long=System.currentTimeMillis())
...
@Database(entities=[SavedWeather::class,Message::class,SavedPlace::class],version=2,exportSchema=false)
abstract class WeatherDb:RoomDatabase() { abstract fun dao():LocalDao
    companion object {
        ...
        fun get(context:Context):WeatherDb = instance ?: synchronized(this) {
            instance ?: Room.databaseBuilder(context.applicationContext,WeatherDb::class.java,"weather.db").addMigrations(MIGRATION_1_2).build().also { instance=it }
        }
    }
}
