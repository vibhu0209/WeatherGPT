...
class WeatherViewModel(app:Application):AndroidViewModel(app) {
    ...
    val messages=repo.dao.messages().stateIn(viewModelScope,SharingStarted.WhileSubscribed(5000),emptyList())
    ...
    fun choose(p:Place) { viewModelScope.launch { repo.dao.savePlace(SavedPlace(repo.key(p),p.name,p.name,p.latitude,p.longitude,p.timezone)); settings.edit{it[stringPreferencesKey("place")]=repo.gson.toJson(p)}; dayOffset=0; results.value=emptyList(); ForecastSync.schedule(getApplication(),value("wifi","true")=="true"); refresh(p) } }
    ...
    fun search(q:String) {
        ...
        searchJob=viewModelScope.launch {
            ...
            try { results.value=repo.api(base()).search(q.trim()).locations; if(results.value.isEmpty()) error.value="no_places" }
            catch(e:CancellationException) { throw e }
            catch(e:Exception) { error.value="search_failed" }
    ...
    fun refresh(p:Place?=place.value) {
        ...
        viewModelScope.launch {
            ...
            try { repo.refresh(p,base()); offline.value=false }
            catch(e:CancellationException) { throw e }
    ...
    fun send(text:String) {
        ...
        viewModelScope.launch {
            ...
            repo.dao.message(Message(role="user",text=text.trim(),language=lang))
            try {
                ...
                repo.dao.message(Message(role="assistant",text=a.answer+"\n\n"+sourceLabel+downloaded+" · "+a.sources.joinToString(),language=a.language))
                offline.value=false
            } catch(e:CancellationException) { throw e }
            catch(e:Exception) {
                offline.value=true
                ...
                repo.dao.message(Message(role="assistant",text=message,language=if(lang=="hi") "hi" else "en"))
            } finally { busy.value=false }
        }
    }
    fun clearChat() { viewModelScope.launch{repo.dao.clearChat(); settings.edit{it.remove(stringPreferencesKey("conversation_id"));it[stringPreferencesKey("day_offset")]="0"}} }
    fun clearAll() { viewModelScope.launch { androidx.work.WorkManager.getInstance(getApplication()).cancelUniqueWork("forecast-sync"); repo.dao.clearChat(); repo.dao.clearWeather(); repo.dao.clearPlaces(); settings.edit{it.clear()}; dayOffset=0 } }
}
