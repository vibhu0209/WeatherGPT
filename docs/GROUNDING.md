...
WeatherGPT grounds numerical answers in typed weather, official-alert, marine, score and climate objects. Provider responses are validated and normalized before they reach chat. Groq may orchestrate WeatherGPT tools and explain verified drafts only after number, unit, named-source and warning-severity validation. Structured JSON remains the source of truth for meteorological values. When Groq is unavailable, deterministic templates answer from the same tools.
