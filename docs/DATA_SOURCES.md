...
| [IMD](https://api.imd.gov.in/public/api_reference.html) | Official Indian authority; access probe only, normalization incomplete | Documented cityforecastloc, station mapping, current weather, warnings and specialist endpoints. Verify provisioned access. | Never substitutes general model data for an official warning. UI says warnings unavailable. |
| [INCOIS](https://incois.gov.in/) | Official ocean information, not integrated | Operational feed/access and licensing still to confirm. | Marine advice explicitly unavailable. |
| [ECMWF Open Data](https://www.ecmwf.int/en/forecasts/datasets/open-data) | Research reference; direct adapter not implemented | Do not count ECMWF twice through intermediary feeds. | No fabricated model provenance. |
...
| [Open-Meteo Marine Weather API](https://open-meteo.com/en/docs/marine-weather-api) | Adapter, fixture tests and live Arabian Sea request passed | Wave height/direction/period, swell and sea-surface temperature for a user-selected sea coordinate. Model guidance, not an official warning. | Coastal accuracy is limited and output is not suitable for navigation; IMD and INCOIS warnings take precedence. |
...
- [BHASHINI compute](https://bhashini.gitbook.io/bhashini-apis/pipeline-compute-call): pipeline discovery and compute credentials still required; adapter not implemented.
